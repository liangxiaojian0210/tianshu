...
**fan-out 定案（2026-09-18 空闲窗验证）**：类型化槽直呼（91ad521）预言 40→28-34 **未中**——空闲窗 10 轮差值模式仍为 **40ns**（31-50 带，6/10 轮恰好 40），指令差未动（+~3k 条/消息，跨运行复现）→ 虚槽接口假设证伪。fanout 残差两刀未中（C1 收件环局部性、虚 pop 接口）后按两轮纪律**停止动刀**：m = **10ns/支**（空闲窗一字不差量级），机制在指令级弥散未完全归因（锁/分发/血缘两侧平价，每支 ~750 条额外指令不集中于单点）。类型化槽作为行为中性改进保留（假设证伪已记录）。**m 预算与 F 语义两项待维护者裁定后，H2 可作正式判决**。
