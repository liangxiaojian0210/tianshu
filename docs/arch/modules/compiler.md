...
**门定义（v3，ADR-0034）**：主承诺为加性分解 `diff ≤ F + k×跳数 + m×扇出支数`——k ≤ 2ns/跳、F ≤ 20ns/消息、m 待定，由空算子五形状同轮差值直接量取（各形状差值 = F + k×(跳数) + m×(支数) 联立拟合 + 边际差分交叉验证）；从承诺为负载条件化百分比（W ≥ (F+k×hops)/1% 时 P99 差 <1%，加性由负载探针保证）。判决报告双轨：百分比 + (F, k, m) 分解；迭代以常数对号。**fan-in 驱动协议（ADR-0037，2026-09-17 等价裁定）**：三方（gold/解释/编译）每源一线程驱动（对齐 ADR-0021 声明语义），gold join 持 std::mutex 手写同步、锁序 j1→j2→sink；线程化形状的 p99/p999 在盾（2 逻辑核）下由调度时间片主导（三方同量级），门比较以 p50 为实现项依据。
