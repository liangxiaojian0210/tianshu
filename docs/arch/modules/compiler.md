...
**有效环境判据**（空闲 + 大核 + performance governor；三轮 p50 应一字不差）。⚠️ 2026-09-17 起金标准补齐 lineage 语义（ADR-0032 D5，同语义基线），旧无血缘基线（50/180/331）作废。**协议 v2（2026-09-17，ADR-0033 D6）**：基准二进制启动即创建并回收一个线程（thread-state warmer），一次性翻转 glibc `__libc_single_threaded`，全进程互斥锁与 shared_ptr 引用计数进入原子路径——生产宿主常态；三方（手写/解释/编译）同态测量，判决不再依赖基准注册顺序（R1 时期 medium/long/fan-in/fan-out 的手写基线本就处于该态，唯 short 是翻转前样本）。**v2 盾内基线（三轮，p50 波动 ≤10ns）**：

| 手写（带血缘，v2 预热态） | short | medium | long | fan-in | fan-out |
|---|---|---|---|---|---|
| p50 | 110 ns | 271 | 651 | 371 | 350-360 |
| p99 | 131 | 310 | 701 | 401 | 491 |

**门定义（v3，ADR-0034）**：主承诺为加性分解 `diff ≤ F + k×跳数 + m×扇出支数`——k ≤ 2ns/跳、F ≤ 20ns/消息、m 待定，由空算子五形状同轮差值直接量取（各形状差值 = F + k×(跳数) + m×(支数) 联立拟合 + 边际差分交叉验证）；从承诺为负载条件化百分比（W ≥ (F+k×hops)/1% 时 P99 差 <1%，加性由负载探针保证）。判决报告双轨：百分比 + (F, k, m) 分解；回炉以常数对号。**fan-in 驱动协议（ADR-0037，2026-09-17 等价裁定）**：三方（gold/解释/编译）每源一线程驱动（对齐 ADR-0021 声明语义），gold join 持 std::mutex 手写同步、锁序 j1→j2→sink；线程化形状的 p99/p999 在盾（2 逻辑核）下由调度时间片主导（三方同量级），门比较以 p50 为实现项依据。
...
- **H2 判决 R1（2026-09-17 盾内三轮）：五形状全 FAIL** — 差值 short +101 / medium +170 / long +210 / fan-in +460 / fan-out +139 ns，折 21-92ns/跳 vs 门槛 ≤2ns/跳，差一个数量级；compiled 对解释执行快 3.2-4.3×（机制收益真实），H1 逐字节一致（9 用例）。**归因完成**（证据 `r1-rounds/attr/`）：差距结构性——入口通用段 + 每跳 fast 段组件 + fan-in 入口放大（三通道轮转 vs 单槽缓存抖动）；测量装置曾受 glibc 单线程闩锁污染（`run_sources` 0ms 创建线程翻转标志，已修 f5399c7），协议 v2 以预热器统一同态。**入口特化已落地（ADR-0033 `SourceEntry`）**：v2 口径下门差值收敛至 20/110/131/170/80 ns。**join 通知链瘦身（2026-09-17 第二轮）**：DataVisitor 的 on_fuse 成员化、notify 只捕 `this`——消除 dispatcher 每 sink 每投递的 std::function 深拷贝（join 链携带 shared_ptr，每消息 4 对原子引用计数）；同轮差值 fan-in 170→150、解释执行 long −170。单槽收件环（depth-1）实验预言未中已按纪律回滚——32 槽环 ≈3-5KB 本就驻留 L1，每跳 inbox 成本是 head/tail 原子对本身（多写者安全契约代价），非缓存局部性。**直槽轮（2026-09-17 ADR-0036，k 最后一刀）**：线性三形状进入与金标准的**噪声带内平价**——p50 同轮差值 short 0-10 / medium 11-29 / long 0-10（其中两轮 long 671=671 完全相等），p99 同轮差值 short −9~+9 / medium +11~+30 / long −10~+20；**k（边际）≈ 0-3ns/跳，达到/贴近 v3 预算（k≤2、F≤20 在轮次噪声 ±10-15 内）**。**fan-in 并发等价轮（ADR-0037，等价裁定「对比的前提是等价」）**：三方每源一线程、gold 补手写同步后，p50 反转为**编译侧大幅领先**——盾内 compiled 371ns vs gold 4338ns（**快 11.7×**：mutex 竞争塌缩 vs 原子 inbox+单飞锁不睡眠），无盾 912 vs 1222 同方向；fan-in 自本轮起**无超额开销**，框架 join 机器对合理手写同步是净优势（原 +120-130「差距」实质是不等价比较）。前史：入口特化（ADR-0033）→ join 通知瘦身 → per-fn 直呼（ADR-0035，k 6-8→4.2）→ 直槽（ADR-0036，k→0-3）→ 并发等价（ADR-0037，fan-in 反转）。

**H2 正式判决（2026-09-18 空闲窗三轮，r2-rounds/verdict-*）：PASS**——五形状同轮差值（compiled − handwritten）**全部 ≤0**：short p50 **-1/-1/-1**（三轮一字不差）/ medium -40~-50 / long -110~-111 / fan-in -3817~-5841 / fan-out -20~-30；p99 除 fan-in r1 一次 +3318（ADR-0037 D3 已归档的调度尾环境项）外全部为负。v3 加性预算（F≤20ns、k≤2ns/跳）以 ≤0 全部满足；负载条件化百分比从承诺在任意 W 下平凡成立。**语义**：编译产物在空算子对抗极端下达到或反超同语义手写装配——fan-in 的领先来自框架 join 机器（原子 inbox + 单飞锁）对合理手写同步（mutex）的真实优势（ADR-0037），线性形状的平价来自 per-fn 直呼 + 直槽就地访问消除了声明层中转开销。回炉战役全程（入口特化 → 通知瘦身 → per-fn → 直槽 → 并发等价 → rvalue → 就地访问）每轮预言对账、未中即停，H1 逐字节等价 14 用例全程锁定。
**fan-out 因果闭环（2026-09-18，8289927）**：残差根因**钉死并修复验证**——①症状 40ns 十轮一字不差；②定位（按指令采样）：`basic_string(&&)` 移动构造份额 7.77% vs 0.89%（≈30ns/消息）；③机制（代码可数）：每消息 17 次 vs 4 次 Lineage 移动（三次中转），SSO 字符串随搬被构造且非内联；④干预 `&&` 传参；⑤预言 25-32；⑥实测 40→20；⑦复验份额 7.77→1.66%——因果闭合。前史：C1 环局部性（未中回滚）、虚 pop 接口（未中，类型化槽中性保留）。

**搬运终刀：直槽就地访问（2026-09-18，2ff5b47）——五形状全部达到或反超手写**。剩余搬运根因 = 槽接口按值取放协议（map 弹出 + sink 弹出物化，每分支 2 次）；修复 = DirectSlot 暴露 `value()` 就地引用（ADR-0036 同栈纪律），map 回调**就地 add_hop**、sink 回调**就地传引用**，非直槽保留弹出路径，join 不变（inbox 弹出是配对语义）。预言 20→5-12，**实测 -10~-21（编译侧反超）**：就地后编译路径构造次数少于手写的"拷一次+移一次"。全形状快照（3 轮）：short -9~+19 / medium -39~-110 / long -110~-140 / fan-in -3417~-6063 / fan-out -10~-20。**H2 从 R1 的 +101~+460 全 FAIL 收敛到五形状 ≤0**（正式判决数字建议空闲窗取，方向已 13 轮一致）。血缘表示按裁定保留字符串（v1 ID 方案否决）。
