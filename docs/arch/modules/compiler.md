...
**fan-out 因果闭环（2026-09-18，8289927）**：残差的差异原因**钉死并修复验证**——①症状：差值 40ns 十轮一字不差；②定位（按指令采样）：`basic_string(&&)` 移动构造指令份额 7.77% vs 0.89%（≈30ns/消息）；③机制（代码可数）：编译侧每消息 17 次 Lineage 移动（槽弹出→局部→值传参 fan→槽压入 三次中转 × 4 分支 + 入口）vs 金标准 4 次（单槽就地写读），SSO 字符串随每次移动被搬运且发射为非内联调用；④干预：`fan()`/`SourceEntry::publish_bytes` 改 `Lineage&&`；⑤预言 25-32；⑥实测 **40 → 20**（模式；高尾为负载漂移）；⑦复验：string 移动份额 **7.77% → 1.66%**——机制指标与结果同步如预测。**剩余 ~20ns**：`DirectSlot::push` 非内联虚调用 ~9-10ns（指令份额 2.48%）+ 轮次噪声带。前史：C1 环局部性（未中回滚）、虚 pop 接口（未中，类型化槽按中性清理保留）、本轮 rvalue 中转砍除（**命中**）。
